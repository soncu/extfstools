...
struct SuperBlock {
  ...
  void dump() const {
    ...
    printf("s_feature_compat=%08x: %s\n", s_feature_compat,
           feat_compat2str(s_feature_compat).c_str());
    printf("s_feature_incompat=%08x: %s\n", s_feature_incompat,
           feat_incompat2str(s_feature_incompat).c_str());
    printf("s_feature_ro_compat=%08x: %s\n", s_feature_ro_compat,
           feat_rocompat2str(s_feature_ro_compat).c_str());
  ...
  static std::string feat_compat2str(uint32_t feat) {
    ...
    if (feat & EXT4_FEATURE_COMPAT_DIR_PREALLOC)
      l.push_back("DIR_PREALLOC");
    if (feat & EXT4_FEATURE_COMPAT_IMAGIC_INODES)
      l.push_back("IMAGIC_INODES");
    if (feat & EXT4_FEATURE_COMPAT_HAS_JOURNAL)
      l.push_back("HAS_JOURNAL");
    if (feat & EXT4_FEATURE_COMPAT_EXT_ATTR)
      l.push_back("EXT_ATTR");
    if (feat & EXT4_FEATURE_COMPAT_RESIZE_INODE)
      l.push_back("RESIZE_INODE");
    if (feat & EXT4_FEATURE_COMPAT_DIR_INDEX)
      l.push_back("DIR_INDEX");
    if (feat & ~all)
      l.push_back(stringformat("unk_%x", feat & ~all));
  ...
  static std::string feat_incompat2str(uint32_t feat) {
    ...
    uint32_t all =
        ...
        EXT4_FEATURE_INCOMPAT_64BIT | EXT4_FEATURE_INCOMPAT_MMP |
        EXT4_FEATURE_INCOMPAT_FLEX_BG | EXT4_FEATURE_INCOMPAT_EA_INODE |
        EXT4_FEATURE_INCOMPAT_DIRDATA | EXT4_FEATURE_INCOMPAT_BG_USE_META_CSUM |
        EXT4_FEATURE_INCOMPAT_LARGEDIR | EXT4_FEATURE_INCOMPAT_INLINE_DATA;
    if (feat & EXT4_FEATURE_INCOMPAT_COMPRESSION)
      l.push_back("COMPRESSION");
    if (feat & EXT4_FEATURE_INCOMPAT_FILETYPE)
      l.push_back("FILETYPE");
    if (feat & EXT4_FEATURE_INCOMPAT_RECOVER)
      l.push_back("RECOVER");
    if (feat & EXT4_FEATURE_INCOMPAT_JOURNAL_DEV)
      l.push_back("JOURNAL_DEV");
    if (feat & EXT4_FEATURE_INCOMPAT_META_BG)
      l.push_back("META_BG");
    if (feat & EXT4_FEATURE_INCOMPAT_EXTENTS)
      l.push_back("EXTENTS");
    if (feat & EXT4_FEATURE_INCOMPAT_64BIT)
      l.push_back("64BIT");
    if (feat & EXT4_FEATURE_INCOMPAT_MMP)
      l.push_back("MMP");
    if (feat & EXT4_FEATURE_INCOMPAT_FLEX_BG)
      l.push_back("FLEX_BG");
    if (feat & EXT4_FEATURE_INCOMPAT_EA_INODE)
      l.push_back("EA_INODE");
    if (feat & EXT4_FEATURE_INCOMPAT_DIRDATA)
      l.push_back("DIRDATA");
    if (feat & EXT4_FEATURE_INCOMPAT_BG_USE_META_CSUM)
      l.push_back("BG_USE_META_CSUM");
    if (feat & EXT4_FEATURE_INCOMPAT_LARGEDIR)
      l.push_back("LARGEDIR");
    if (feat & EXT4_FEATURE_INCOMPAT_INLINE_DATA)
      l.push_back("INLINE_DATA");
    if (feat & ~all)
      l.push_back(stringformat("unk_%x", feat & ~all));
    return JoinStringList(l, ",");
  }
  static std::string feat_rocompat2str(uint32_t feat) {
    ...
    if (feat & EXT4_FEATURE_RO_COMPAT_SPARSE_SUPER)
      l.push_back("SPARSE_SUPER");
    if (feat & EXT4_FEATURE_RO_COMPAT_LARGE_FILE)
      l.push_back("LARGE_FILE");
    if (feat & EXT4_FEATURE_RO_COMPAT_BTREE_DIR)
      l.push_back("BTREE_DIR");
    if (feat & EXT4_FEATURE_RO_COMPAT_HUGE_FILE)
      l.push_back("HUGE_FILE");
    if (feat & EXT4_FEATURE_RO_COMPAT_GDT_CSUM)
      l.push_back("GDT_CSUM");
    if (feat & EXT4_FEATURE_RO_COMPAT_DIR_NLINK)
      l.push_back("DIR_NLINK");
    if (feat & EXT4_FEATURE_RO_COMPAT_EXTRA_ISIZE)
      l.push_back("EXTRA_ISIZE");
    if (feat & EXT4_FEATURE_RO_COMPAT_QUOTA)
      l.push_back("QUOTA");
    if (feat & EXT4_FEATURE_RO_COMPAT_BIGALLOC)
      l.push_back("BIGALLOC");
    if (feat & EXT4_FEATURE_RO_COMPAT_METADATA_CSUM)
      l.push_back("METADATA_CSUM");
    if (feat & ~all)
      l.push_back(stringformat("unk_%x", feat & ~all));
    return JoinStringList(l, ",");
  }
...
struct Inode {
  ...
  void dump() const {
    printf("m:%06o %4d o[%5d %5d] t[%10d %10d %10d %10d]  %12" PRId64
           " [b:%8d] F:%05x(%s) X:%08x %s\n",
           i_mode, i_links_count, i_gid, i_uid, i_atime, i_ctime, i_mtime,
           i_dtime, datasize(), i_blocks, i_flags, fl2str(i_flags).c_str(),
           i_file_acl, hexdump(i_osd2, 12).c_str());
    if ((i_mode & 0xf000) == EXT4_S_IFLNK && i_size < 60) {
      printf("symlink: %s\n", symlink.c_str());
    } else if (i_flags & EXT4_EXTENTS_FL) {
      e.dump();
  ...
  static std::string fl2str(uint32_t fl) {
    ...
    uint32_t all = EXT4_SECRM_FL | EXT4_UNRM_FL | EXT4_COMPR_FL | EXT4_SYNC_FL |
                   EXT4_IMMUTABLE_FL | EXT4_APPEND_FL | EXT4_NODUMP_FL |
                   EXT4_NOATIME_FL | EXT4_DIRTY_FL | EXT4_COMPRBLK_FL |
                   EXT4_NOCOMPR_FL | EXT4_ECOMPR_FL | EXT4_INDEX_FL |
                   EXT4_IMAGIC_FL | EXT4_JOURNAL_DATA_FL | EXT4_NOTAIL_FL |
                   EXT4_DIRSYNC_FL | EXT4_TOPDIR_FL | EXT4_HUGE_FILE_FL |
                   EXT4_EXTENTS_FL | EXT4_EA_INODE_FL | EXT4_EOFBLOCKS_FL |
                   EXT4_INLINE_DATA_FL | EXT4_RESERVED_FL;
    ...
    if (fl & EXT4_SYNC_FL)
      l.push_back("SYNC");
    if (fl & EXT4_IMMUTABLE_FL)
      l.push_back("IMMUTABLE");
    if (fl & EXT4_APPEND_FL)
      l.push_back("APPEND");
    if (fl & EXT4_NODUMP_FL)
      l.push_back("NODUMP");
    if (fl & EXT4_NOATIME_FL)
      l.push_back("NOATIME");
    if (fl & EXT4_DIRTY_FL)
      l.push_back("DIRTY");
    if (fl & EXT4_COMPRBLK_FL)
      l.push_back("COMPRBLK");
    if (fl & EXT4_NOCOMPR_FL)
      l.push_back("NOCOMPR");
    if (fl & EXT4_ECOMPR_FL)
      l.push_back("ECOMPR");
    if (fl & EXT4_INDEX_FL)
      l.push_back("INDEX");
    if (fl & EXT4_IMAGIC_FL)
      l.push_back("IMAGIC");
    if (fl & EXT4_JOURNAL_DATA_FL)
      l.push_back("JOURNAL_DATA");
    if (fl & EXT4_NOTAIL_FL)
      l.push_back("NOTAIL");
    if (fl & EXT4_DIRSYNC_FL)
      l.push_back("DIRSYNC");
    if (fl & EXT4_TOPDIR_FL)
      l.push_back("TOPDIR");
    if (fl & EXT4_HUGE_FILE_FL)
      l.push_back("HUGE_FILE");
    if (fl & EXT4_EXTENTS_FL)
      l.push_back("EXTENTS");
    if (fl & EXT4_EA_INODE_FL)
      l.push_back("EA_INODE");
    if (fl & EXT4_EOFBLOCKS_FL)
      l.push_back("EOFBLOCKS");
    if (fl & EXT4_INLINE_DATA_FL)
      l.push_back("INLINE_DATA");
    if (fl & EXT4_RESERVED_FL)
      l.push_back("RESERVED");
    if (fl & ~all)
      l.push_back(stringformat("unk_%x", fl & ~all));
    return JoinStringList(l, ",");
  }
...
struct BlockGroup {
  ...
  void parse(uint64_t first, const SuperBlock &super,
    ...
    itableoffset =
        first +
        (bgdesc.bg_inode_table % super.s_blocks_per_group) * super.blocksize();
...
struct Ext2FileSystem {
  ...
  void parse(ReadWriter_ptr r) {
    ...
    //   with smaller blocksize, the groupdescs would overlap with the first
    //   superblock
    //
    r->setpos(1024);
    super.parse(r);
  ...
  Inode getinode(uint32_t nr) const {
    ...
    if (nr >= super.s_inodes_count)
      nr = 0;
    return groups[nr / super.s_inodes_per_group].getinode(
        nr % super.s_inodes_per_group);
  }
};
...
struct exportdirectory : action {
  ...
  void perform(Ext2FileSystem &fs) override {
    ...
    recursedirs(fs, ino, ".", [&](const DirectoryEntry &e,
...
struct listfiles : action {
  void perform(Ext2FileSystem &fs) override {
    ...
    // tree.scanfs(fs);
    recursedirs(fs, ROOTDIRINODE, "", [](const DirectoryEntry &e,
                                         const std::string &path) {
...
struct dumpblocks : action {
  ...
  void perform(Ext2FileSystem &fs) override {
    for (uint32_t nr = first; nr < last; nr++) {
      printf("block %x\n", nr);
      printf("%s\n",
             hexdump(0, &fs.super.getblock(nr).front(), fs.super.blocksize())
                 .c_str());
    }
  }
};
...
public:
  SparseReader(ReadWriter_ptr r) : r(r), _off(0) {
    readheader();
    scansparse();
...
private:
  void readheader() {
    ...
    checksum = r->read32le();
    printf(
        "sparse: v%08x h:%04x c:%04x b:%08x nblk:%08x ncnk:%08x,  cksum:%08x\n",
  ...
  std::map<uint64_t, sparserecord>::const_iterator findofs(uint64_t ofs) const {
    ...
    if (i->first + i->second.ndwords * 4 < ofs) {
      // printf("%" PRId64 " outside of block: ", ofs);
      // i->second.dump(i->first);
      return _map.end();
    }
...
public:
  virtual size_t read(uint8_t *p, size_t n) {
    ...
    while (n) {
      ...
      if (i == _map.end())
        break;
      size_t want =
          std::min((uint64_t)n, i->second.ndwords * 4 - (_off - i->first));
      ...
      if (i->second.isfill) {
        ...
        // printf("found %" PRId64 " : %zx bytes from %" PRId64 "+%" PRId64 "-%"
  ...
  virtual void setpos(uint64_t off) { _off = off; }
  virtual void truncate(uint64_t off) { throw "truncate not implemented"; }
  virtual uint64_t size() { return blkcount * blksize; }
  virtual uint64_t getpos() const { return _off; }
  virtual bool eof() { return getpos() >= size(); }
};
void usage() {
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    ReadWriter_ptr r;
    if (openasblockdev)
      r = boost::shared_ptr<BlockDevice>(
          new BlockDevice(fsfile, BlockDevice::readonly));
    else
      r = boost::shared_ptr<MmapReader>(
          new MmapReader(fsfile, MmapReader::readonly));
    if (!offsets.empty()) {
      r = boost::make_shared<OffsetReader>(r, offsets.front(),
                                           r->size() - offsets.front());
      offsets.pop_front();
    }
