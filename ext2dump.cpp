...
struct SuperBlock {
  ...
  void parse(const uint8_t *first, size_t size) {
    ...
    s_inodes_count = get32le(p);
    p += 4; // 0000;
    s_blocks_count = get32le(p);
    p += 4; // 0004;
    s_r_blocks_count = get32le(p);
    p += 4; // 0008;
    s_free_blocks_count = get32le(p);
    p += 4; // 000c;
    s_free_inodes_count = get32le(p);
    p += 4; // 0010;
    s_first_data_block = get32le(p);
    p += 4; // 0014;
    s_log_block_size = get32le(p);
    p += 4; // 0018;
    s_log_frag_size = get32le(p);
    p += 4; // 001c;
    s_blocks_per_group = get32le(p);
    p += 4; // 0020;
    s_frags_per_group = get32le(p);
    p += 4; // 0024;
    s_inodes_per_group = get32le(p);
    p += 4; // 0028;
    s_mtime = get32le(p);
    p += 4; // 002c;
    s_wtime = get32le(p);
    p += 4; // 0030;
    s_mnt_count = get16le(p);
    p += 2; // 0034;
    s_max_mnt_count = get16le(p);
    p += 2; // 0036;
    s_magic = get16le(p);
    p += 2; // 0038;
    s_state = get16le(p);
    ...
    s_errors = get16le(p);
    p += 2; // 003c;
    s_minor_rev_level = get16le(p);
    p += 2; // 003e;
    s_lastcheck = get32le(p);
    p += 4; // 0040;
    s_checkinterval = get32le(p);
    p += 4; // 0044;
    s_creator_os = get32le(p);
    p += 4; // 0048;
    s_rev_level = get32le(p);
    p += 4; // 004c;
    s_def_resuid = get16le(p);
    p += 2; // 0050;
    s_def_resgid = get16le(p);
    p += 2; // 0052;

    s_first_ino = get32le(p);
    p += 4; // 0054;
    s_inode_size = get16le(p);
    p += 2; // 0058;
    s_block_group_nr = get16le(p);
    p += 2; // 005a;
    s_feature_compat = get32le(p);
    p += 4; // 005c;
    s_feature_incompat = get32le(p);
    p += 4; // 0060;
    s_feature_ro_compat = get32le(p);
    p += 4; // 0064;
    memcpy(s_uuid, p, 16);
    p += 16; // 0068;
    memcpy(s_volume_name, p, 16);
    p += 16; // 0078;
    memcpy(s_last_mounted, p, 64);
    p += 64; // 0088;
    s_algo_bitmap = get32le(p);
    p += 4; // 00c8;
  }
  ...
  void dump() const {
    ...
    printf("i->%d, b->%d groups\n", s_inodes_count / s_inodes_per_group,
...
struct ExtentLeaf : ExtentNode {
  ...
  ExtentLeaf(const uint8_t *first) { parse(first); }
  virtual void parse(const uint8_t *first) {
    ...
    ee_len = get16le(p);
    p += 2;
    ee_start_hi = get16le(p);
    p += 2;
    ee_start_lo = get32le(p);
    p += 4;
  }
...
struct ExtentInternal : ExtentNode {
  ...
  ExtentInternal(const uint8_t *first) { parse(first); }

  virtual void parse(const uint8_t *first) {
    const uint8_t *p = first;
    ei_block = get32le(p);
    p += 4;
    ei_leaf_lo = get32le(p);
    p += 4;
    ei_leaf_hi = get16le(p);
    p += 2;
    ei_unused = get16le(p);
    p += 2;
  }
...
struct ExtentHeader {
  ...
  void parse(const uint8_t *first) {
    ...
    eh_magic = get16le(p);
    p += 2;
    eh_entries = get16le(p);
    p += 2;
    eh_max = get16le(p);
    p += 2;
    eh_depth = get16le(p);
    p += 2;
    eh_generation = get32le(p);
    p += 4;
  }
  void dump() const {
    printf("M%04x, N%d,max%d, d:%d, g:%d\n", eh_magic, eh_entries, eh_max,
           eh_depth, eh_generation);
  }
};
...
...
struct Inode {
  ...
  bool _empty;
  void parse(const uint8_t *first, size_t size) {
    _empty = std::find_if(first, first + size,
                          [](uint8_t b) { return b != 0; }) == first + size;
    ...
    i_generation = get32le(p);
    p += 4;
    i_file_acl = get32le(p);
    p += 4;
    i_dir_acl = get32le(p);
    p += 4;
    i_faddr = get32le(p);
  ...
  void dump() const {
    printf("m:%06o %4d o[%5d %5d] t[%10d %10d %10d %10d]  %12lld [b:%8d] "
           "F:%05x X:%08x %s\n",
           i_mode, i_links_count, i_gid, i_uid, i_atime, i_ctime, i_mtime,
           i_dtime, datasize(), i_blocks, i_flags, i_file_acl,
           hexdump(i_osd2, 12).c_str());
    if ((i_mode & 0xf000) == 0xa000 && i_size < 60) {
      printf("symlink: %s\n", symlink.c_str());
    } else if (i_flags & 0x80000) {
      e.dump();
  ...
  bool enumi1block(const SuperBlock &super, uint64_t &bytes,
                   const uint8_t *pblock, BLOCKCALLBACK cb) const {
    if (pblock == NULL)
      return true;
  ...
  bool enumi2block(const SuperBlock &super, uint64_t &bytes,
                   const uint8_t *pblock, BLOCKCALLBACK cb) const {
    if (pblock == NULL)
      return true;
    const uint32_t *p = (const uint32_t *)pblock;
  ...
  bool enumi3block(const SuperBlock &super, uint64_t &bytes,
                   const uint8_t *pblock, BLOCKCALLBACK cb) const {
    if (pblock == NULL)
      return true;
    const uint32_t *p = (const uint32_t *)pblock;
...
struct BlockGroupDescriptor {
  ...
  void parse(const uint8_t *first) {
    ...
    bg_block_bitmap = get32le(p);
    p += 4;
    bg_inode_bitmap = get32le(p);
    p += 4;
    bg_inode_table = get32le(p);
    p += 4;
    bg_free_blocks_count = get16le(p);
    p += 2;
    bg_free_inodes_count = get16le(p);
    p += 2;
    bg_used_dirs_count = get16le(p);
    p += 2;
    bg_pad = get16le(p);
    p += 2;
...
struct BlockGroup {
  ...
  void parse(const uint8_t *first, const SuperBlock &super,
             BlockGroupDescriptor &bgdesc) {
    bbitmap.parse(first +
                      (bgdesc.bg_block_bitmap % super.s_blocks_per_group) *
                          super.blocksize(),
                  super.blocksize());
    ibitmap.parse(first +
                      (bgdesc.bg_inode_bitmap % super.s_blocks_per_group) *
                          super.blocksize(),
                  super.blocksize());
    parseitable(first +
                    (bgdesc.bg_inode_table % super.s_blocks_per_group) *
                        super.blocksize(),
  ...
  template <typename FUNC> void enuminodes(uint32_t inodebase, FUNC cb) const {
  ...
  static size_t dumpdirent(const uint8_t *first) {
    const uint8_t *p = first;
    uint32_t inode = get32le(p);
    p += 4;
    uint32_t rec_len = get16le(p);
    p += 2;
    uint32_t name_len = get8(p);
    p += 1;
    uint32_t filetype = get8(p);
    p += 1;
    printf("%8d %o '%s'\n", inode, filetype,
           std::string((const char *)p, name_len).c_str());
    return rec_len;
  }
...
struct Ext2FileSystem {
  ...
  template <typename FUNC> void enuminodes(FUNC cb) const {
    uint32_t inodebase = 1;
    for (unsigned i = 0; i < groups.size(); i++) {
  ...
  const Inode &getinode(uint32_t nr) const {
    ...
    if (nr >= super.s_inodes_count)
      nr = 0;
    return groups[nr / super.s_inodes_per_group]
        .inodetable[nr % super.s_inodes_per_group];
  }
};

struct DirectoryEntry {
  uint32_t inode;
  uint8_t filetype; // 1=file, 2=dir, 3=chardev, 4=blockdev, 5=fifo, 6=sock,
                    // 7=symlink
  std::string name;
...
struct TreeReconstructor {
  ...
  template <typename FN> void recursedirs(uint32_t nr, std::string path, FN f) {
...
uint32_t searchpath(Ext2FileSystem &fs, uint32_t nr, std::string path) {
  ...
  i.enumblocks(fs.super, [&](const uint8_t *first) -> bool {
    ...
    while (p < first + fs.super.blocksize()) {
      ...
      if (path[e.name.size()] == '/' &&
          std::equal(e.name.begin(), e.name.end(), path.begin())) {
        if (e.filetype == 2) {
...
struct exportinode : action {
  ...
  exportinode(uint32_t nr, const std::string &savepath)
      : nr(nr), savepath(savepath) {}
...
struct hexdumpfile : action {
  ...
  hexdumpfile(const std::string &ext2path) : ext2path(ext2path) {}
...
struct exportdirectory : action {
  ...
  void perform(Ext2FileSystem &fs) override {
    ...
    recursedirs(
        fs, ino, ".", [&](const DirectoryEntry &e, const std::string &path) {
          if (e.filetype == 2) {
...
struct listfiles : action {
  void perform(Ext2FileSystem &fs) override {
    ...
    // tree.scanfs(fs);
    recursedirs(
        fs, 2, "", [](const DirectoryEntry &e, const std::string &path) {
          printf("%9d %s%c %s/%s\n", e.inode, e.filetype >= 8 ? "**" : "",
                 "0-dcbpsl"[e.filetype & 7], path.c_str(), e.name.c_str());
        });
  }
};
struct dumpfs : action {
  void perform(Ext2FileSystem &fs) override { fs.dump(); }
};
struct dumpblocks : action {
  uint32_t first;
  uint32_t last;
  dumpblocks(uint32_t first, uint32_t last) : first(first), last(last) {}
  void perform(Ext2FileSystem &fs) override {
    for (uint32_t nr = first; nr < last; nr++) {
      printf("block %x\n", nr);
      printf("%s\n",
             hexdump(0, fs.super.getptrforblock(nr), fs.super.blocksize())
                 .c_str());
    }
  }
};
action::ptr action::parse(const std::string &arg) {
  ...
  if (arg[0] == '#') {
    ...
    if (ix == arg.npos) {
      ...
      return boost::make_shared<exportinode>(strtoul(&arg[1], 0, 0),
                                             arg.substr(ix + 1));
    }
  } else {
    ...
    // /path/:outdir
    if (ix > 0 && arg[ix - 1] == '/')
      return boost::make_shared<exportdirectory>(arg.substr(0, ix - 1),
                                                 arg.substr(ix + 1));
    else
      return boost::make_shared<exportfile>(arg.substr(0, ix),
                                            arg.substr(ix + 1));
  }
}
...
void expandsparse(const uint8_t *first, const uint8_t *last, ByteVector &data) {
  ...
  uint32_t magic = get32le(p);
  p += 4;
  if (magic != 0xed26ff3a)
    throw "invalid magic";
  uint32_t version = get32le(p);
  p += 4;
  uint16_t hdrsize = get16le(p);
  p += 2;
  uint16_t cnkhdrsize = get16le(p);
  p += 2;
  uint32_t blksize = get32le(p);
  p += 4;
  uint32_t blkcount = get32le(p);
  p += 4;
  uint32_t chunkcount = get32le(p);
  p += 4;
  uint32_t checksum = get32le(p);
  p += 4;
  ...
  while (p + cnkhdrsize <= last) {
    ...
    case 0xcac1:
      ...
      std::copy_n((const uint32_t *)p, (chunksize - cnkhdrsize) / 4,
                  (uint32_t *)q);
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (argv[i][0] == '-')
      switch (argv[i][1]) {
      case 'l':
        actions.push_back(boost::make_shared<listfiles>());
        break;
      case 'd':
        actions.push_back(boost::make_shared<dumpfs>());
        break;
      case 'o':
        offsets.push_back(getintarg(argv, i, argc));
        break;
      case 'b': {
